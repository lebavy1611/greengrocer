...
class OrderController extends ApiController
{
    ...
    {
        try {
            $order = Order::with(['user', 'paymentMethod:id,name', 'processStatus:id,name', 'coupon'])->orderFilter($request)->orderBy('created_at', 'desc')->paginate(config('paginate.number_orders'));
    ...
    {
        try {
            $order = Order::with(['user', 'paymentMethod:id,name', 'processStatus:id,name', 'coupon', 'orderDetails.product'])->findOrFail($id);
